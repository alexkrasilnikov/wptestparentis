...
<!--ВЫвод категроий курсов-->
<?php
// Получаем список всех рубрик (категорий курсов)
$categories = get_categories(array(
    'taxonomy'   => 'course-category', // замените 'course-category' на таксономию, используемую для категорий курсов в вашей установке Tutor LMS
    'hide_empty' => 0, // показываем даже пустые категории
));

// Если есть рубрики
if (!empty($categories)) {
    echo '<div class="course-categories">';
	 echo '<div><a class="cat-link" href="' . home_url('/courses') . '">Все курсы</a></div>';
    foreach ($categories as $category) {
        echo '<div><a class="cat-link" href="' . get_category_link($category->term_id) . '">' . $category->name . '</a></div>';
    }
    echo '</div>';
}
?>
<h3 id="active-category"></h3>
<script>
document.addEventListener('DOMContentLoaded', function() {
    ...
    var categoryLinks = document.querySelectorAll('.course-categories a');

    // Перебираем ссылки
    categoryLinks.forEach(function(link) {
        // Получаем URL ссылки
        var categoryUrl = link.href;

        // Если URL ссылки соответствует текущему URL страницы
        if (categoryUrl === currentPageUrl) {
            // Получаем текст ссылки (название категории)
            var categoryName = link.textContent;

            // Добавляем класс "active" к активной ссылке
            link.classList.add('active');

            // Выводим название активной категории
            document.getElementById('active-category').textContent = categoryName;

            // Прерываем цикл, так как уже нашли активную категорию
            return;
        }
    });
});
