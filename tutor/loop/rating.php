...
<?php
                ...
                // Получаем список категорий для курса
                $categories = wp_get_post_terms($course_id, 'course-category');

                // Если есть категории у курса
                if (!empty($categories)) {
                    echo '<div class="course-categories-title">';
                    foreach ($categories as $category) {
                        // Выводим название категории
                        echo '<span class="course-category">' . $category->name . '</span>';
                    }
                    echo '</div>';
                }
                ?>
