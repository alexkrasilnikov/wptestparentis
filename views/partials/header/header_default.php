...
<header class="header-default">
	...
	<nav class="navbar <?php echo tutorstarter_header_switcher(); ?> <?php echo 'navbar-right' === tutorstarter_header_switcher() ? 'has-search-field' : ''; ?>">
		...
		<div class="navbar-utils">
			<?php
			if ( class_exists( 'WooCommerce' ) && 'header_fullwidth_center' !== get_theme_mod( 'header_type_select' ) ) :
				global $woocommerce;
			...
			<?php if ( ! is_user_logged_in() || is_customize_preview() ) : ?>
				<?php if ( true === get_theme_mod( 'cta_text_toggle', true ) ) : ?>
					<div class="utils-btn"><a class="btn-getstarted" href="<?php echo esc_url( home_url() . '/login' ); ?>"><?php printf( esc_html__( '%s', 'tutorstarter' ), get_theme_mod( 'cta_text', 'Войти' ) ); ?></a></div>
				<?php endif; ?>
			<?php endif; ?>
		</div>
	</nav>
